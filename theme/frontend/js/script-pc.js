...
if($('#wrap_banner_fixed')[0]){

    var banner_fixed,h_banner_fixed,dk_banner_fixed;

    banner_fixed = $('#wrap_banner_fixed');
    h_banner_fixed = banner_fixed.offset().top;

    function funcBannerFixed() {

        dk_banner_fixed = ($(this).scrollTop() >= h_banner_fixed);

        // console.log(dk_banner_fixed);

        if (dk_banner_fixed === true) {
            banner_fixed.addClass("banner_sticky");
        } else {
            banner_fixed.removeClass("banner_sticky");
        }
    }

    $(window).on('scroll resize', funcBannerFixed);
};
